                               ...
                               float *output )
{
	...
	if (tid >= filterRadius && tid < dataSize-filterRadius) {
		for ( int k = 0; k < filterWidth; k++ )
            num += filter[k] * data[ tid - filterRadius + k ];
	}
	output[tid] = num;
                               ...
                               float *output )
{
	...
	// Add filter
	if (tx >= 0 && tx < filterWidth) {
		filterS[tx] = filter[tx]; 
	}

	// Add data
	dataS[tx + BLOCK_SIZE] = data[tid];
	// ensure that it doesn't exceed somehow
	if (tid - BLOCK_SIZE >= 0) 
		dataS[tx] = data[tid - BLOCK_SIZE];
	if (tid + BLOCK_SIZE < dataSize) 
		dataS[tx + 2*BLOCK_SIZE] = data[tid + BLOCK_SIZE];
	__syncthreads();

	float num = 0.0; // thing to sum up
	if (tid >= filterRadius && tid < dataSize-filterRadius) {
		int base = tx + BLOCK_SIZE - filterRadius; // starting point
		for ( int k = 0; k < filterWidth; k++ )
            num += filterS[k] * dataS[ base + k ];
	}
	output[tid] = num; // allocate after done
}
